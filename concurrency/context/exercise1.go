...
func findUser(rw http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		log.Println("goroutine for the database call is terminating")
	}()
	...
	case u := <-chu:
		...
		log.Println("Sent response with StatusOk")
	...
	case <-ctx.Done():
		...
		log.Println("Sent response with StatusRequestTimeout")
