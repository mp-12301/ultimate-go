...
func (s sysadmin) administrate(system string) {
	fmt.Printf("admin %v administering system %v\n", s.name, system)
}
...
func (p programmer) develop(system string) {
	fmt.Printf("programmer %v coding system %v\n", p.name, system)
}
...
func main() {
	...
	devs.Enqueue(programmer{name: "programmer a"})
	devs.Enqueue(programmer{name: "programmer b"})
